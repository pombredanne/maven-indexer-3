...
@SpringApplicationConfiguration(classes = { TestUnauthenticatedConfig.class, TestApp.class })
@WebAppConfiguration
...
public class CustomComparatorTest {
	...
	public void testUnauthenticatedMavenRepository() throws Exception {
		mockMvc.perform(get("/maven/unauthid/artifactid/index.yml").headers(TestMocks.GOOD_AUTH_HEADERS)).andDo(print())
				.andExpect(status().isOk());
		mockMvc.perform(get("/maven/unauthid/artifactid/index.yml").headers(TestMocks.BAD_AUTH_HEADERS)).andDo(print())
				.andExpect(status().isOk());
		mockMvc.perform(get("/maven/unauthid/artifactid/index.yml")).andDo(print()).andExpect(status().isOk());
	}
}
